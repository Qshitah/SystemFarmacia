...
const addOrderAsync = createAsyncThunk(
...
const updateOrderAsync = createAsyncThunk(
    ...
    try {
      if (
        ...
        const response = await axios.put(
          `http://localhost:8080/api/orders/${orderData.orderId}`,
          orderData,
          {
            headers: {
...
const deleteOrderAsync = createAsyncThunk(
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(addOrderAsync.fulfilled, (state, action) => {
        state.statusOrders = 'succeeded';
        state.orders = [...state.orders, action.payload]; // Add the new order to the state
      })
      ...
      .addCase(updateOrderAsync.fulfilled, (state, action) => {
        ...
        state.orders = state.orders.map((order) =>
          order.orderId === action.payload.id ? action.payload : order,
        );
      })
